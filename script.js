/* ==========================================================
   Bar Centrale — script.js
   - Scroll animation Canvas (food-rustic, 151 frame, cover §4.2)
   - IntersectionObserver: .fade-up, .stagger-card
   - Parallax (desktop only, max 80px, 1 foto ambiente)
...
(function () {
  ...
  // Scroll Animation — food-rustic (§4.2 cover)
  ...
  if (canvas && section) {
    ...
    var FRAME_PATH = 'https://8ispuxmgjxgu2r5q.public.blob.vercel-storage.com/templates/bar-001/frames/';
    var FRAME_COUNT = 151;
  ...
  // Parallax — desktop only, max 80px
  if (!window.matchMedia('(max-width: 767px)').matches) {
    var pTiles = document.querySelectorAll('.ambiente-tile');
    var pTarget = pTiles.length > 1 ? pTiles[1] : null;
    if (pTarget) {
      var pImg = pTarget.querySelector('img');
      if (pImg) {
        window.addEventListener('scroll', function () {
          ...
          pImg.style.transform = 'translateY(' + ((p - 0.5) * 80) + 'px)';
        }, { passive: true });
      }
    }
  }
  ...
  if ('IntersectionObserver' in window) {
    ...
    var stagObs = new IntersectionObserver(function (entries) {
      entries.forEach(function (e) {
        if (e.isIntersecting) {
          var d = parseInt(e.target.dataset.stagger || 0, 10) * 120;
